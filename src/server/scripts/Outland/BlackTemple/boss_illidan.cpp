...
struct boss_illidan_stormrage : public BossAI
{
    ...
    void JustEngagedWith(Unit* /*who*/) override
    {
        ...
        if (GameObject* musicController = instance->GetGameObject(DATA_ILLIDAN_MUSIC_CONTROLLER))
            musicController->PlayDirectSound(EVENT_BT_SUMMIT_WALK_3_SOUND_ID);
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            switch (eventId)
            {
                ...
                case EVENT_MOVE_TO_WARGLAIVE_POINT:
                {
                    ...
                    if (GameObject* musicController = instance->GetGameObject(DATA_ILLIDAN_MUSIC_CONTROLLER))
                        musicController->PlayDirectSound(EVENT_BT_STORM_WALK_HERO_2_SOUND_ID);
                    break;
                }
                ...
                case EVENT_RESUME_COMBAT:
                    ...
                    if (GameObject* musicController = instance->GetGameObject(DATA_ILLIDAN_MUSIC_CONTROLLER))
                        musicController->PlayDirectSound(EVENT_BT_STORM_WALK_UNI_3_SOUND_ID);
                    break;
                ...
                case EVENT_DEFEATED_TEXT:
                    ...
                    if (GameObject* musicController = instance->GetGameObject(DATA_ILLIDAN_MUSIC_CONTROLLER))
                        musicController->PlayDirectSound(EVENT_BT_ARRIVAL_WALK_HERO_1_SOUND_ID);
            ...
            switch (eventId)
            {
                ...
                case EVENT_AKAMA_START_SOUND:
                    if (GameObject* musicController = _instance->GetGameObject(DATA_ILLIDAN_MUSIC_CONTROLLER))
                        musicController->PlayDirectSound(EVENT_BT_SUMMIT_WALK_SOUND_ID);
                    break;
...
class spell_illidan_flame_blast : public SpellScript
{
    ...
    void HandleBlaze(SpellEffIndex /*effIndex*/, int32& /*damage*/)
    {
        Unit* target = GetHitUnit();
